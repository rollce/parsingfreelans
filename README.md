...
2. `Accounts` — список бирж, карточка каждой биржи, включение/выключение мониторинга, logout-сессии.
3. `Profile` — общий профиль (имя/резюме/аватар/портфолио).
4. `Settings` — пауза и auto-apply.
5. `Run cycle` — ручной запуск цикла парсинга.
...
2. Ты оцениваешь результат через inline-кнопки `Хорошо / Good` и `Плохо / Bad`.
