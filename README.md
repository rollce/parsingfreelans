...
3. `Аккаунты` — список бирж, карточка каждой биржи, включение/выключение мониторинга, удаление сессии.
4. `Профиль` — общий профиль (имя/резюме/аватар/портфолио).
5. `Настройки` — пауза, автоотклик, фильтры, режим сканирования, языковой режим.
6. `Запустить цикл` — ручной запуск цикла парсинга.
