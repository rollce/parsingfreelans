...
async def stats() -> dict:
    ...
    validator = await worker._get_effective_validation_settings()
    validator_spike = await worker._get_validator_spike_snapshot()
    suspend_until = await worker._get_auto_apply_suspend_until()
    ...
    data["proposal_validation_enabled"] = bool(validator["enabled"])
    data["proposal_min_chars"] = int(validator["min_chars"])
    data["proposal_max_chars"] = int(validator["max_chars"])
    data["proposal_similarity_threshold"] = float(validator["similarity_threshold"])
    data["proposal_similarity_window"] = int(validator["similarity_window"])
    data["validator_spike_alert_enabled"] = bool(validator_spike["enabled"])
    data["validator_spike_threshold"] = int(validator_spike["threshold"])
    data["validator_spike_window_minutes"] = int(validator_spike["window_minutes"])
    data["validator_spike_count"] = int(validator_spike["count"])
    data["validator_spike_active"] = bool(validator_spike["active"])
    data["auto_apply_suspend_until"] = suspend_until.isoformat() if suspend_until else None
